    ...
    def grade_problem(self, student_dir, t_output, t_res, commands, commands_ref, exact, error_func): 
        ...
        try:
            ...
            for i in range(0, n):
                ...
                try:
                    result = np.genfromtxt(os.path.join(student_dir, t_res[i]), delimiter=",", dtype=str, encoding='ISO-8859-1')
                ...
                try:
                    expected = np.genfromtxt(t_output[i], delimiter=",", dtype=str, encoding='ISO-8859-1')
                except Exception as err:
